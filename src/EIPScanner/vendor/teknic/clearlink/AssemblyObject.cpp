...
#include "EIPScanner/vendor/teknic/clearlink/AssemblyObject.h"
...
namespace clearlink
{
    AssemblyObject::AssemblyObject(eipScanner::cip::CipUint instanceId,
    ...
    void AssemblyObject::_getAttributes()
    {
        std::vector<ReflexiveFieldReference> classAttributes = _getAttributeReferences();

        const uint8_t assemblyOneSDInput_AttributeId = 0x3; 
        ...
        auto getAttributeResponse = _messageRouter->sendRequest(_sessionInfo, eipScanner::cip::ServiceCodes::GET_ATTRIBUTE_SINGLE,
                                                                eipScanner::cip::EPath(getClassId(), getInstanceId(), assemblyOneSDInput_AttributeId));
    ...
    void AssemblyObject::_setAllAttributesIndividually()
    {
        std::vector<ReflexiveFieldReference> classAttributes = _getAttributeReferences();
